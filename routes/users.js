...
router.get('/authors', async(req, res) => {
  console.log("testing in server all")
	  try {
const subscribers = await Subscriber.find({SubmittedRecipes: {$gt : 0}});
console.log(subscribers)
 res.json(subscribers)
 console.log("suceessallauthors")
  } catch (err) {
 res.status(500).json({ message: err.message })
  }
})
...
router.put('/addfriend', async(req, res) => {
  ...
  var subscribers = await Subscriber.find().where('UserID').equals(req.body.UserID)
  ...
  try {
  //console.log(subscribers)
    if (req.body.FriendUserID != null) {
      if(!subscriber.Friends.includes(req.body.FriendUserID))
        {
      subscriber.Friends.push( req.body.FriendUserID);
  ...
  console.log(subscriber)
     ...
     // console.log(req.params['id'])
      var filter = { UserID:req.body.UserID };
    var update = { $set: {Friends:subscriber.Friends  } };
      let doc = await Subscriber.updateOne(filter, update );
      console.log("step3")
      res.status(204).json({ message: "Success" });
    }
      else{
      res.status(400).json({ message: "User Id is already added" })
    }
    }
    else{
...
console.log(useridvar);
  ...
  try{
   var subscribers = await Subscriber.find().where('UserID').equals(useridvar);
   console.log(subscribers)
   var subscribersf = subscribers[0];
...
router.get('/getbyemail/:emailid', cors(),async(req, res) => {
  ...
  var emailid = req.params['emailid'];
    try{
    var  subscribers = await Subscriber.find().where('EmailId').equals(emailid);
     subscribers = subscribers[0];
    console.log(subscribers)
    res.json(subscribers)
  }
...
console.log(subscriber)
  try {
  var  subscribers = await Subscriber.find().where('EmailId').equals(subscriber.EmailId);
    subscribers = subscribers[0];
    console.log(subscribers)
    if(subscribers == undefined)
    {
