...
///*****************testfetchrecipebyuser
router.get("/byuser", cors(), async (req, res) => {
  console.log("inside byuserfetchrecipe");
...
router.get("/search", cors(), async (req, res) => {
  ...
  try {
    for (let index = 0; index < keyword_array.length; index++) {
      ...
      console.log(filter);
      var subscribers = await Subscriber.find(filter);
...
router.post("/", async (req, res) => {
  ...
  try {
    console.log("tyty1");
    subscribersret = await Subscriber.find()
      .where("dbcolumn")
      .sort({ Recipeid: -1 })
      .limit(1);
    console.log("tyty2");
